...
<body>
    <div class="container">
        ...
        <div class="toolbar">
            ...
            <span class="summary-text">
                Có <strong><%= phongTrongList.size() %></strong> phòng trống sẵn sàng đặt
            </span>
        </div>

        <% if (message != null) { %>
            <div class="alert <%= message.contains("thành công") ? "alert-success" : "alert-error" %>">
                <i class="fa-solid <%= message.contains("thành công") ? "fa-circle-check" : "fa-circle-exclamation" %>"></i>
                <%= message %>
            </div>
        <% } %>
                ...
                <div class="room-card">
                    <div class="room-card-header">
                        <span class="room-number">
                            <i class="fa-solid fa-door-open"></i> Phòng <%= p.getSoPhong() %>
                        </span>
                        <span class="room-status">Trống</span>
                    </div>
                    <div class="room-card-body">
                        <div class="room-info">
                            <i class="fa-solid fa-tag"></i>
                            <span>Loại phòng:</span>
                            <span class="value"><%= p.getTenLoaiPhong() %></span>
                        </div>
                        <div class="room-info">
                            <i class="fa-solid fa-users"></i>
                            <span>Sức chứa:</span>
                            <span class="value"><%= p.getSoNguoiToiDa() %> người</span>
                        </div>
                        <div class="room-info">
                            <i class="fa-solid fa-money-bill-wave"></i>
                            <span>Giá/đêm:</span>
                        </div>
                        <div class="room-price"><%= nf.format(p.getGiaCoBan()) %> VNĐ</div>
                    </div>
                    <button class="btn-book"
                            onclick="openBookingModal(<%= p.getMaPhong() %>, <%= p.getSoPhong() %>, '<%= p.getTenLoaiPhong() %>', '<%= nf.format(p.getGiaCoBan()) %>')">
                        <i class="fa-solid fa-calendar-check"></i> Đặt phòng này
                    </button>
                </div>
                <% } %>
            </div>
    ...
    <div id="bookingModal" class="modal">
        <div class="modal-content">
            ...
            <h3><i class="fa-solid fa-calendar-plus"></i> Xác nhận đặt phòng</h3>
            ...
            <form action="BookingServlet" method="POST">
                ...
                <input type="hidden" id="modalMaPhong" name="maPhong">
                ...
                <div class="form-group">
                    <label>Ghi chú</label>
                    <textarea name="ghiChu" placeholder="Ghi chú thêm (không bắt buộc)..."></textarea>
                </div>

                <button type="submit" class="btn-submit">
                    <i class="fa-solid fa-check"></i> Xác nhận đặt phòng
                </button>
            </form>
        </div>
    </div>
